...
def load_cached_csv(ticker: str, data_dir: Path | None = None) -> tuple[pd.DataFrame, float] | tuple[None, None]:
    """
    Load a pre-committed CSV as fallback when yfinance is unavailable.
    Recovers spot price from moneyness: S = K / exp(moneyness).
    Returns (df, spot) or (None, None) if file not found.
    """
    if data_dir is None:
    ...
    try:
        ...
        required = {"strike", "moneyness", "days_to_expiry", "implied_vol", "option_type"}
        if not required.issubset(df.columns) or df.empty:
            return None, None
        row0 = df.iloc[0]
        spot = float(row0["strike"]) / np.exp(float(row0["moneyness"]))
        return df, round(spot, 2)
    except Exception:
        return None, None
...
def fetch_option_chain(ticker: str = "SPY", r: float = RF_RATE) -> pd.DataFrame:
    """
    Fetch live option chain from Yahoo Finance. Raises RuntimeError on failure
    (instead of sys.exit) so callers can catch and fall back to cached CSV.
    """
    sep = "-" * 55
    print(f"\n{sep}")
    ...
    try:
        ...
        raise RuntimeError(f"Cannot fetch spot price for {ticker}: {exc}") from exc

    try:
        expirations = tk.options
    except Exception as exc:
        raise RuntimeError(f"Cannot fetch expirations for {ticker}: {exc}") from exc
